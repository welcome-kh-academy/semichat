...
public class Paging {
	 ...
	 private int totalCount; // 총 게시글 수(DB에서 조회된 전체 결과의 행수)
...
//	ex)
//	<1 2 3 4 5> 이렇게 있으면 pageCount는 5
	...
	private void makePaging() {
		...
		//기본값 설정
		if(curPage == 0) setCurPage(1);			//첫 페이지 기본 값 세팅
		if(pageCount == 0) setPageCount(10);	//화면에 보여질 페이지 수 기본 값 세팅
		if(listCount == 0) setListCount(10);	//화면에 보여질 게시글 수 기본 값 세팅
		...
		//현재 페이지 보정
		//총 페이지 번호보다 현재 페이지 번호가 높을 때 총 페이지 번호로 고정
